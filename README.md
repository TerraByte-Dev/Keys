...
- **Sheet music you already own** — import **MusicXML** (`.musicxml` or `.mxl`) and Keys engraves it with Verovio and plays it, with a real transport: play, pause, rewind, a tempo you can drag, and a bar you can click to jump anywhere. Every notation program exports it.
...
- **62 presets, ~190 instruments** — presets are plain JSON you can edit by hand. The instrument browser enumerates every preset the SoundFont actually contains — including all 13 drum kits — rather than trusting the GM chart.
...
The full suite is ten checks and 721 assertions; see [`CONTRIBUTING.md`](CONTRIBUTING.md). There is no
